...
LockManager::LockManager(deque<TxnProto*>* ready_txns)
  : ready_txns_(ready_txns) {
  for (int i = 0; i < TABLE_SIZE; i++)
    lock_table_[i] = new deque<LockRequest>();
}

int LockManager::Lock(TxnProto* txn) {
  int not_acquired = 0;
...
void LockManager::Release(TxnProto* txn) {
  for (int i = 0; i < txn->read_set_size(); i++)
    if (IsLocal(txn->read_set(i)))
...
void LockManager::Release(const Key& key, TxnProto* txn) {
